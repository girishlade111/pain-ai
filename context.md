...
## P14 End-to-End Validation (2026-09-25 — live isolated sidecar + generation + security battery, NO arch changes)
- Live: sidecar booted on isolated home (healthz v0.1.0/sha e2168f71/manual/local); chat "Hello" → honest missing-key SSE error (no fabricated turn); memory write→read→process-restart→read green; Bearer 401/404 matrix green (approve unknown-id 404, bad-token 401).
- Generation: TXT+PDF+XLSX+PPTX via Hermes skill helpers + 4-file app scaffold with verified artifact group — 20/20 PASS (magic bytes, library reopen, containment, traversal reject).
- Security battery 12/12 (obfuscated/encoded/cradle/pipe shells, deny precedence, persist-refuses-keys, traversal, bare-dot); suites: Python 127+1skip (venv) · tsc 0 · vite clean · cargo check clean. Blocked on-box: with-model turns (no BYOK/local server), Rust test exec + installer smoke (Smart App Control), provider-restart persistence (Rust-only path).
